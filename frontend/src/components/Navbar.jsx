...
import { Search, Bell, ChevronDown, LogOut, User, Plus, X } from 'lucide-react';
    ...
    <nav className={`fixed top-0 left-0 right-0 z-40 transition-colors duration-500 px-4 md:px-12 py-3 flex items-center justify-between ${
      ...
      <div className="flex items-center space-x-8">
        ...
        <ul className="hidden md:flex items-center space-x-6 text-sm text-gray-300">
          <li 
            onClick={() => navigate('/browse')}
            className={`cursor-pointer transition hover:text-white ${location.pathname === '/browse' ? 'text-white font-semibold' : ''}`}
          >
            Home
          </li>
          <li 
          ...
          <li 
            ...
            className={`cursor-pointer transition hover:text-white ${location.pathname === '/my-list' ? 'text-white font-semibold' : ''}`}
          >
            My List
          </li>
        </ul>
      ...
      <div className="flex items-center space-x-4 md:space-x-6">
        ...
        <div className="relative flex items-center">
          <button 
            ...
            className="p-1 text-gray-200 hover:text-white transition focus:outline-none"
            aria-label="Search"
          >
          ...
          {showSearch && (
            <div className="flex items-center bg-black/80 border border-gray-600 rounded px-2 py-1 ml-2 transition-all duration-300">
              <input
                type="text"
        ...
        {/* Notifications */}
        <button className="text-gray-200 hover:text-white transition hidden sm:block relative">
        ...
        <div className="relative">
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-3 w-56 bg-black/95 border border-gray-800 rounded-md shadow-2xl py-2 z-50 divide-y divide-gray-800 text-sm">
              ...
              <div className="pt-2 px-3">
                <button
                  ...
                  className="w-full flex items-center space-x-2 text-left py-2 px-2 text-xs text-gray-300 hover:text-red-500 hover:bg-gray-900 rounded transition"
                >
